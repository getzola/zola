...
rather than `.html`. This may be particularly useful if you'd like to include headings generated by the shortcode in the
[table of contents](@/documentation/content/table-of-contents.md).

If you wish to use something similar to shortcodes in your templates, you can use [Tera macros](https://keats.github.io/tera/docs#macros). They are functions or components that you can call to return some text.
...
In both cases, the arguments must be named and will be passed to the template.
Parentheses are mandatory even if there are no arguments.

Note that while shortcodes look like normal Tera expressions, they are not Tera at all -- they effectively shuttle arguments to their template. Several limitations of note are:
...
Simply call the shortcode as if it were a Tera function in a variable block.
...
When one of these variables conflicts with a variable passed as argument, the argument value will be used.
...
If you really need that, you can rewrite your Markdown content to pass `lang` as an argument to the shortcode.

Every shortcode can access the current language in the `lang` variable in the context.
This is useful for presenting/filtering information in a shortcode depending on the current language. For example, to display a per-language book cover for the current page in a shortcode called `bookcover.md`:
...
A useful attribute to `page` in shortcodes is `colocated_path`. This is used when you want to pass the name of some assets to shortcodes without repeating the folder's full path. You may find it most useful when combining it with `load_data` or `resize_image`.
...
Here are some sample shortcodes for inspiration.
