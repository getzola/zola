...
This means that you can, for example, access the content of your `_index.md` files
using the `{{ section.content }}` variable in `index.html` or `section.html`,
whereas you would use `{{ page.content }}` in `page.html`.  The page and section
variables are described in more detail in the next section of this documentation.
...
directory.  These custom templates will not be used by default (with one exception 
described below).  Instead, the custom template will _only_ be used if you apply it
by setting the `template` front-matter variable to the path for that template (or if
you `include` it in another template that is applied).  For example, if you created a
custom template for your site's About page called `about.html`, you could apply it to
your `about.md` page by including the following front matter in your `about.md` page:
...
The one time that custom templates _are_ applied without being specified in the front
matter of a Markdown file is if the template is named `rss.xml`, `sitemap.xml`, or
`robots.txt`.  If a template with any of those names is found, it will be automatically 
applied to the relevant page, overwriting the built-in template for that page.  See
the relevant section of this documentation for more details.
