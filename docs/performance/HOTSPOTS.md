...
| PERF-002 | `giallo-0.5.2 pattern_set.rs:21` via `markdown::render_content` | `Mutex<RegSet>` around every Oniguruma match | once per code-block token | O(matches) forced serial | profile: 80.6% of busy CPU blocked (markdown-heavy-4k) | **fixed upstream: −61% wall on markdown-heavy, see OPTIMIZATIONS.md** | **patch ready** |
