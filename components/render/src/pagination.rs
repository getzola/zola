...
mod tests {
    ...
    fn test_can_build_paginator_context() {
        ...
        assert_eq!(context.get(&tera::value::Key::from("paginate_by")).unwrap(), &Value::from(2));
        assert_eq!(
            context.get(&tera::value::Key::from("first")).unwrap(),
            &Value::from("https://vincent.is/posts/")
        );
        assert_eq!(
            context.get(&tera::value::Key::from("last")).unwrap(),
            &Value::from("https://vincent.is/posts/page/2/")
        );
        assert_eq!(context.get(&tera::value::Key::from("previous")).unwrap(), &Value::none());
        assert_eq!(
            context.get(&tera::value::Key::from("next")).unwrap(),
            &Value::from("https://vincent.is/posts/page/2/")
        );
        assert_eq!(context.get(&tera::value::Key::from("current_index")).unwrap(), &Value::from(1));
        assert_eq!(
            context.get(&tera::value::Key::from("pages")).unwrap().as_array().unwrap().len(),
            2
        );
        ...
        assert_eq!(context.get(&tera::value::Key::from("next")).unwrap(), &Value::none());
        assert_eq!(
            context.get(&tera::value::Key::from("previous")).unwrap(),
            &Value::from("https://vincent.is/posts/")
        );
        assert_eq!(context.get(&tera::value::Key::from("current_index")).unwrap(), &Value::from(2));
        assert_eq!(context.get(&tera::value::Key::from("total_pages")).unwrap(), &Value::from(3));
        assert_eq!(
            context.get(&tera::value::Key::from("pages")).unwrap().as_array().unwrap().len(),
            1
        );
    ...
    fn works_with_empty_paginate_path() {
        ...
        let context = context.as_map().unwrap();
        assert_eq!(
            context.get(&tera::value::Key::from("base_url")).unwrap(),
            &Value::from("https://vincent.is/posts/")
        );
    }
