...
impl<'a> State<'a> {
    ...
    pub fn render(mut self, content: &'a str, ctx: &MarkdownContext) -> Result<Rendered> {
        self.output.reserve(content.len() / 10);
        for event in Parser::new_ext(content, ctx.options()) {
    ...
    fn end_heading(&mut self, ctx: &MarkdownContext) {
        ...
        let id = if let Some(ref explicit) = h.explicit_id {
            if self.anchors.contains(&explicit) {
        ...
        self.output.push(Event::Html(format!("</h{}>", h.level).into()));
    ...
    fn start_link(
        ...
        if dest_url.is_empty() {
            self.errors.push(Error::msg("Link missing URL"));
    ...
    fn end_image(&mut self, ctx: &MarkdownContext) {
        ...
        let html = if ctx.config.markdown.lazy_async_image {
            format!(
                r#"<img src="{src}"{title_attr} alt="{alt}" loading="lazy" decoding="async" />"#
            )
        } else {
            format!(r#"<img src="{src}"{title_attr} alt="{alt}" />"#)
        };
        self.push_html(html);
    }
