...
/// Colocated asset links refers to files in the same directory.
fn is_colocated_asset_link(link: &str) -> bool {
    !link.starts_with('/') && !link.starts_with("..") && !link.starts_with('#') && !STARTS_WITH_SCHEMA_RE.is_match(link)
}
...
mod tests {
    ...
    #[test]
    fn test_is_colocated_asset_link_true() { let links: [&str; 2] = ["./same-dir.md", "file.md"];
