...
mod tests {
    ...
    fn can_resize_image() {
        ...
        assert_eq!(
            data.get(&Key::from("static_path")).unwrap(),
            &Value::from(format!(
                "{}",
    ...
    fn can_get_image_metadata() {
        ...
        let data = static_fn.call(kwargs, &state).unwrap();
        let data = data.as_map().unwrap();
        assert_eq!(data.get(&Key::from("height")).unwrap(), &Value::from(380));
        assert_eq!(data.get(&Key::from("width")).unwrap(), &Value::from(300));
        assert_eq!(data.get(&Key::from("format")).unwrap(), &Value::from("jpg"));
        assert_eq!(data.get(&Key::from("mime")).unwrap(), &Value::from("image/jpeg"));
